...
Although we briefly introduced functions in section 3.2, this section provides an overview from a different perspective. It has been retained here, though summarized wherever appropriate.
...
>Let’s look at what statements and expressions are and how their differences affect the bodies of functions. Because Rust is an expression-based language, this is an important distinction to understand.
...
>**Function definitions are also statements. As we’ll see shortly, calling a function is not a statement, though.**

>Statements do not return values. Therefore, you can’t assign a `let` statement to another variable, as Code 3.4.3 tries to do; you’ll get an error (C-Output 3.4.3):
...
>Expressions can be part of statements: In Code 3.4.3, the `6` in the statement `let y = 6;` is an expression that evaluates to the value `6`. **Calling a function is an expression. Calling a macro is an expression. A new scope block created with curly brackets is an expression.** Consider Code 3.4.4.
...
>This **expression**: `{ let x = 3; x + 1 }` is a block that, in this case, evaluates to `4`. That value gets bound to `y` as part of the `let` statement. Note the `x + 1` line without a semicolon at the end, which is unlike most of the lines you’ve seen so far. Expressions do not include ending semicolons. If you add a semicolon to the end of an expression, you turn it into a statement, and it will then not return a value.
...
Code 3.4.5 below is an example of a function that returns a value (the **expression** `5`). If you compile and run Code 3.4.5, Rust will execute it without any issues and produce E-Output 3.4.5. However, if you try adding a semicolon (`;`) after the `5`, the compiler will throw the error shown in C-Output 3.4.5.
